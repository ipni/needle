...
needle is a server implementing the [Delegated Routing V1 HTTP API](https://specs.ipfs.tech/routing/http-routing-v1/).
...
**NDJSON responses stream.** A record reaches the client as soon as needle has
...
**needle is tuned for browsers.** A browser client has a small request budget
and a person waiting on it. needle spends its own resources to save the
...
first loses every record needle resolved, and reads it as no providers. See
...
## needle is a proxy: pass through what you do not understand

needle sits between clients and other routing systems. Anything it does not
recognize belongs to somebody else, so it forwards it untouched rather than
dropping it or normalizing it. needle is not the only thing that will read
these records.
...
**Unknown multiaddr components.** An address may carry protocols needle has no
...
fails to parse in go-multiaddr, so boxo rejects the record before needle sees
it. needle can only pass through what it can parse, and widening that is a
go-multiaddr concern, not a needle one.
...
needle wires together the delegated routing server and clients from
...
protocol-level changes on IPIPs. needle keeps only what is needle-specific:
...
the durable fix lives in boxo (ipfs/boxo#1166), needle keeps a local re-check
