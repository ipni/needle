...
needle is a caching delegated routing proxy, not a libp2p node. It answers
...
- **Latency.** A cached answer returns in microseconds; a DHT walk takes
  seconds.
- **Stable, reachable peers.** The cache holds peers that needle has recently
...
> flag (on by default). With `--dht=disabled`, needle is a plain proxy:
> `/providers` and `/peers` forward to the delegated HTTP endpoints and no
> address caching takes place.

## The two stores

needle keeps peer addresses in two places, both consulted through one read
...
Addresses enter the cached address book whenever needle sees a peer: a
...
- **Failure** doubles a backoff (1h, 2h, 4h, ...). After repeated failures past
  48 hours, needle evicts the peer.
...
When an identify completes (organically or via a probe), needle replaces the
...
needle handles these addresses on their own terms:
...
needle returns it as is and caches the observed addresses. When a record has no
addresses, needle consults the cache; on a miss it dispatches a background
...
Watch `needle_cached_router_find_peer_lookups_rejected` in
...
needle checks the cache first and returns immediately on a hit, without
...
As a proxy, needle returns a fast answer built from peers it knows are
...
Several things needle does not do look like oversights. They are choices, and
...
needle could return the peer ID alone and let the client resolve it, and the
...
needle does not honor it today, because the cache fallback drops those records
before the filter layer ever sees them.

The reason is what a client does next. Its only move is to call
`/routing/v1/peers/{peerid}`, which reaches the same needle that just failed
...
needle pays for every walk anyway. Resolving once on the server and sharing
the answer through the cache spends needle's resources instead of the
browser's, and needle has more of them. See [who this is tuned
...
would send it after a peer needle already knows is unreachable.
...
on a clean shutdown, and restored before needle serves its first request. It
...
backoff state, so a restart does not re-dial peers needle has already given
up on.
...
`NEEDLE_DHT_CRAWL_SNAPSHOT_MAX_AGE`. It saves what the crawl found - each peer
...
needle passes no datastore to the DHT, so value records, including IPNS
