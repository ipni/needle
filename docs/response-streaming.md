...
A `/routing/v1` request can take seconds. needle sends results as it finds
them rather than waiting for all of them, so a client can start dialing the
first provider while needle is still resolving the rest.
...
needle is tuned for browser clients, and a browser works under tighter limits
...
Results go out as needle finds them, so a client can act on the first usable
...
peer IDs, because needle resolves them once and shares the answer through its
...
needle holds the response open while background address lookups are still
...
connection is open, more records may still arrive. Once it closes, needle is
done. An empty NDJSON stream that closes immediately means needle found
nothing, not that it gave up early.

There is no "retry after" hint for streams. NDJSON sends its headers with the
first record, before needle knows whether later lookups will succeed, so no
...
needle routing   |----------------------------|
                                                ^        ^
                                    needle flushes      client gives up
```

needle stops its routing lookups at
[`NEEDLE_ROUTING_TIMEOUT`](environment-variables.md#needle_routing_timeout),
...
starts counting before needle does, because the request has to reach needle
first.

If both used the same value, a slow lookup would end with the client aborting
at the same moment needle was about to send. Every record needle resolved
...
> [!IMPORTANT]
> Raising `NEEDLE_ROUTING_TIMEOUT` to or above the client deadline does not
...
back. needle compresses responses, and compression middleware usually waits
...
place, a provider needle had already resolved would sit in the buffer until a
...
needle therefore compresses from the first byte (`MinSize(0)` in
...
needle returns providers as it finds them, so results arrive early and in
...
Each of those lookups costs needle a DHT walk, and a failed walk caches
nothing, so the next client repeats it. Bound the concurrency client side and
prefer the addresses needle already returned.
...
- [environment-variables.md](environment-variables.md): `NEEDLE_ROUTING_TIMEOUT`
