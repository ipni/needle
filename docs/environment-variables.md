...
The hold is bounded: after 8 below-floor fires the cut fires anyway, so a request that never reaches the floor still ends at most `9 × SOMEGUY_DHT_TAIL_BUDGET` after the last non-DHT router finishes, rather than living indefinitely. A request served by the DHT alone is still never cut, at any floor.
